...
export default function Player({
  ...
  const crouchingColliderRef = useRef<RapierCollider>(null);

  const visualRef = useRef<THREE.Group>(null);

  const playerEffectState = useRef<PlayerEffectSnapshot>({
    x: -10,
    footY: 2.1,
    z: 0,
    velocityX: 0,
  ...
  const pushTransitionStateRef = useRef<"none" | "start" | "pushing" | "stop">(
    "none",
  );
  ...
  // 1 = ขวา
  // -1 = ซ้าย
  const facingDirection = useRef<1 | -1>(1);
  ...
  const setCrouchingColliderRef = useCallback(
    (collider: RapierCollider | null) => {
      ...
      if (isCrouching.current) {
        ...
        collider.setCollisionGroups(INACTIVE_COLLISION_GROUPS);
  ...
  const currentAnimation = useRef<PlayerAnimation>("Idle");

  function changeAnimation(nextAnimation: PlayerAnimation) {
  ...
  useFrame((_, delta) => {
    ...
    if (mapExitTransition?.active) {
      ...
      const steps = mapExitTransition.steps;

      const currentStep = steps[mapExitStepIndex.current];
      ...
      if (visualRef.current) {
        visualRef.current.rotation.y = currentStep.rotationY;
      }
      ...
      if (mapExitTimer.current >= currentStep.duration) {
        mapExitTimer.current = 0;
    ...
    const isTouchingDownThisFrame =
      !wasGrounded.current && stableGrounded.current;
    ...
    if (mapEnterTransition?.active) {
      ...
      const steps = mapEnterTransition.steps;

      const currentStep = steps[mapEnterStepIndex.current];
      ...
      if (!currentStep) {
        mapEnterCompleted.current = true;
      ...
      mapEnterTimer.current += safeDelta;
      ...
      if (mapEnterTimer.current >= currentStep.duration) {
        mapEnterTimer.current = 0;
    ...
    if (controlsLocked) {
       ...
       * ถ้ายืนปกติให้ Idle
       */
      if (stableGrounded.current && !isCrouching.current) {
        changeAnimation("Idle");
      }
    ...
    if (!prevIsPushingRef.current && isPushing) {
      pushTransitionStateRef.current = "start";

      pushTransitionTimerRef.current = 0.6;
      ...
      facingDirection.current = pushState.facingDirection;

      if (visualRef.current) {
        visualRef.current.rotation.y =
          pushState.facingDirection > 0 ? Math.PI / 2 : -Math.PI / 2;
      }
    } else if (prevIsPushingRef.current && !isPushing) {
      pushTransitionStateRef.current = "stop";

      pushTransitionTimerRef.current = 0.5;
    }

    prevIsPushingRef.current = isPushing;

    if (pushTransitionTimerRef.current > 0) {
      pushTransitionTimerRef.current -= safeDelta;

      if (pushTransitionTimerRef.current <= 0) {
        pushTransitionTimerRef.current = 0;

        if (pushTransitionStateRef.current === "start") {
          pushTransitionStateRef.current = "pushing";
        } else if (pushTransitionStateRef.current === "stop") {
          pushTransitionStateRef.current = "none";
        }
      }
    }
    ...
    const wantsCrouchedSprint =
      keys.current.crouch && keys.current.run && isMoving;
    ...
    if (
      ...
      if (!isCrouching.current) {
        ...
        facingDirection.current = requestedFacingDirection;

        if (visualRef.current) {
          visualRef.current.rotation.y =
            requestedFacingDirection > 0 ? Math.PI / 2 : -Math.PI / 2;
        }

        startGroundTransition("CrouchedSpinting");
      }
    }
    ...
    if (
      ...
      if (
        ceilingContacts.current === 0 &&
        (groundTransition.current === null ||
          groundTransition.current === "CrouchedStanding")
      ) {
        ...
        if (!isCrouching.current) {
          ...
          if (shouldPlayManualStand) {
            startGroundTransition("CrouchedStanding");
          }
        }
      }
    }
    ...
    if (isMoving) {
      ...
      runStopInputTimer.current += safeDelta;
    } else {
      runStopInputTimer.current = 0;
    }
     ...
     */
    if (jumpQueued.current && groundTransition.current !== null) {
      clearGroundTransition();
    }

    const activeGroundTransition = groundTransition.current;

    if (
      activeGroundTransition !== null &&
      !groundTransitionStartedThisFrame.current &&
      currentAnimation.current === activeGroundTransition
    ) {
      groundTransitionTimer.current += safeDelta;

      const transitionDuration =
        GROUND_TRANSITION_DURATIONS[activeGroundTransition];

      if (groundTransitionTimer.current >= transitionDuration) {
        clearGroundTransition();
    ...
    const movementSmoothing = 1 - Math.exp(-14 * safeDelta);

    let velocityX = THREE.MathUtils.lerp(
      currentVelocity.x,
      targetVelocityX,
      movementSmoothing,
    );
    ...
    if (stableGrounded.current && !didJump.current) {
      lastGroundFootY.current = body.translation().y - PLAYER_FOOT_OFFSET;
    }
    ...
    const currentHorizontalSpeed = Math.abs(currentVelocity.x);
    ...
    if (
      ...
      !isClimbing.current &&
      !isRunJumpingUp.current &&
      currentHorizontalSpeed >= SLIDE_MIN_ENTRY_SPEED
    ) {
      const nextSlideDirection = currentVelocity.x >= 0 ? 1 : -1;

      slideDirection.current = nextSlideDirection;
      facingDirection.current = nextSlideDirection;

      if (visualRef.current) {
        visualRef.current.rotation.y =
          nextSlideDirection > 0 ? Math.PI / 2 : -Math.PI / 2;
      }
    ...
    if (isSliding.current) {
      ...
      const speedAlongSlide = startedSlideThisFrame
        ? Math.max(currentHorizontalSpeed, SLIDE_MIN_INITIAL_SPEED)
        : Math.max(0, currentVelocity.x * slideDirection.current);

      const nextSlideSpeed =
        speedAlongSlide * Math.exp(-SLIDE_DRAG * safeDelta);

      velocityX = slideDirection.current * nextSlideSpeed;

      const shouldEndSlide =
        !stableGrounded.current ||
        nextSlideSpeed <= SLIDE_BLOCKED_SPEED ||
    ...
    if (ledgeGrabCooldown.current > 0) {
      ledgeGrabCooldown.current -= safeDelta;
    }
    ...
    if (isHanging.current) {
      ...
      if (jumpQueued.current || climbInputQueued.current) {
      ...
      if (dropInputQueued.current) {
        dropFromLedgeQueued.current = true;
      }
    }
    ...
    if (isHanging.current && dropFromLedgeQueued.current) {
    ...
    if (isHangDropping.current) {
      hangDropTimer.current += safeDelta;

      if (hangDropTimer.current >= HANG_DROP_DURATION) {
    ...
    if (isHanging.current && climbQueued.current && !isEnteringHang.current) {
      climbQueued.current = false;
      dropFromLedgeQueued.current = false;
      ...
      const facing = facingDirection.current;

      const top = ledgeTopPosition.current;
      ...
      climbEndPosition.current = {
        x: top.x + facing * CLIMB_FORWARD_DISTANCE,
    ...
    if (isClimbing.current) {
      climbTimer.current += safeDelta;

      const progress = Math.min(climbTimer.current / CLIMB_DURATION, 1);
      ...
      if (progress < 0.65) {
        ...
        x = THREE.MathUtils.lerp(
          climbStartPosition.current.x,
          climbUpPosition.current.x,
          smooth,
        );

        y = THREE.MathUtils.lerp(
          climbStartPosition.current.y,
          climbUpPosition.current.y,
          smooth,
        );
      } else {
        /*
        ...
        x = THREE.MathUtils.lerp(
          climbUpPosition.current.x,
          climbEndPosition.current.x,
          smooth,
        );

        y = THREE.MathUtils.lerp(
          climbUpPosition.current.y,
          climbEndPosition.current.y,
          smooth,
        );
      }
      ...
      if (progress >= 1) {
        ...
        body.setTranslation(climbEndPosition.current, true);
    ...
    if (isRunJumpingUp.current) {
      runJumpUpTimer.current += safeDelta;

      const progress = Math.min(
        runJumpUpTimer.current / RUN_JUMP_UP_DURATION,
        1,
      );
      ...
      if (progress < 0.6) {
        ...
        nextPosition = {
          x: runJumpUpStartPosition.current.x,
          y: THREE.MathUtils.lerp(
            runJumpUpStartPosition.current.y,
            runJumpUpTopPosition.current.y,
            smooth,
          ),
          z: 0,
        };
        ...
        nextPosition = {
          x: THREE.MathUtils.lerp(
            runJumpUpTopPosition.current.x,
            runJumpUpEndPosition.current.x,
            smooth,
          ),
          y: runJumpUpEndPosition.current.y,
          z: 0,
        };
      }
      ...
      if (progress >= 1) {
        ...
        body.setTranslation(runJumpUpEndPosition.current, true);
        body.setGravityScale(1, true);
    ...
    if (isHanging.current) {
      if (isEnteringHang.current) {
        hangEntryTimer.current += safeDelta;

        if (hangEntryTimer.current >= JUMP_HANG_DURATION) {
          isEnteringHang.current = false;
          hangEntryTimer.current = 0;
        }
      }
       ...
       * ล็อก Body ไว้ที่ขอบ
       */
      body.setTranslation(hangPosition.current, true);
    ...
    // ============================
      ...
      !isRunJumpingUp.current &&
      (!grounded ||
        (didJump.current && velocityY > TAKEOFF_GROUND_IGNORE_VELOCITY)) &&
      (velocityY <= 0 ||
        (didJump.current && velocityY > TAKEOFF_GROUND_IGNORE_VELOCITY)) &&
      ...
      const wallRay = new rapier.Ray(
        {
          x: playerPosition.x,
          y: playerPosition.y + LEDGE_LOWER_RAY_Y,
          z: playerPosition.z,
        },
        {
          x: facing,
      ...
      const upperRay = new rapier.Ray(
        {
          x: playerPosition.x,
          y: playerPosition.y + LEDGE_UPPER_RAY_Y,
          z: playerPosition.z,
        },
        {
          x: facing,
       ...
       */
      if (wallHit && !upperHit) {
        const wallPoint = wallRay.pointAt(wallHit.timeOfImpact);
        ...
        const topRay = new rapier.Ray(
          {
            /*
             * ข้ามเข้าไปด้านใน
             * platform นิดหนึ่ง
             */
            x: wallPoint.x + facing * 0.12,

            y: playerPosition.y + LEDGE_TOP_RAY_Y,
        ...
        if (topHit) {
          const topPoint = topRay.pointAt(topHit.timeOfImpact);
          ...
          const passiveFallDistance = Math.max(
            0,
            lastGroundFootY.current - currentFootY,
          );

          // ขอบที่กำลังตรวจอยู่เป็นขอบเดิม
          // ที่เราเพิ่งเดินตกลงมาหรือไม่
          const isSameLevelLedge =
            Math.abs(ledgeHeight) <= SAME_LEDGE_HEIGHT_TOLERANCE;
           ...
           */
          const canGrabLedgeFromFall =
            didJump.current ||
            (passiveFallDistance >= MIN_PASSIVE_LEDGE_GRAB_FALL_DISTANCE &&
              !isSameLevelLedge);

          const shouldRunJumpUp =
            didJump.current && ledgeHeight <= RUN_JUMP_UP_MAX_LEDGE_HEIGHT;
          ...
          if (shouldRunJumpUp) {
            ...
            runJumpUpEndPosition.current = {
              x: wallPoint.x + facing * (PLAYER_RADIUS + 0.12),
              y: targetBodyY,
              z: 0,
            };
            ...
            hangPosition.current = {
              x: wallPoint.x - facing * HANG_DISTANCE_FROM_WALL,
              y: topPoint.y - HANG_BODY_BELOW_LEDGE,
              z: 0,
            };
          ...
          if (shouldRunJumpUp || isHanging.current) {
            velocityX = 0;
            velocityY = 0;
    ...
    const groundedBeforeJump = stableGrounded.current;
    ...
    if (
      ...
      jumpStartedRunning.current =
        currentAnimation.current === "Jog" ||
        currentAnimation.current === "Spint" ||
        (isMoving && keys.current.run);
    ...
    const animationGrounded = jumpedThisFrame ? false : stableGrounded.current;

    const highFallBeforeGroundReset = highFallActive.current;
    ...
    if (animationGrounded || isHanging.current || isClimbing.current) {
      highFallActive.current = false;
    }
    ...
    if (!animationGrounded && velocityY < 0) {
      const playerPosition = body.translation();
      ...
      const maxRayDistance =
        PLAYER_FOOT_OFFSET +
        Math.max(LAND_PREP_DISTANCE, HIGH_FALL_MIN_CLEARANCE);
      ...
      const hasWalkableGround =
        hit !== null && hit.normal.y >= MIN_GROUND_SUPPORT_NORMAL_Y;

      const distanceFromFeet = hasWalkableGround
        ? hit.timeOfImpact - PLAYER_FOOT_OFFSET
        : Number.POSITIVE_INFINITY;

      shouldPreLand =
        hasWalkableGround && distanceFromFeet <= LAND_PREP_DISTANCE;
      ...
      if (
        !didJump.current &&
        velocityY <= HIGH_FALL_START_VELOCITY &&
        distanceFromFeet > HIGH_FALL_MIN_CLEARANCE
      ) {
     ...
     * แปลว่าเพิ่งแตะพื้น
     */
    const justLanded = !wasGrounded.current && animationGrounded;

    if (justLanded) {
      landingAnimation.current = highFallBeforeGroundReset
        ? "HardLanding"
        : "Landing";

      landingTimer.current = highFallBeforeGroundReset
        ? HARD_LANDING_DURATION
        : LAND_DURATION;
    ...
    if (
      landingTimer.current > 0 &&
      !justLanded &&
      currentAnimation.current === landingAnimation.current
    ) {
      landingTimer.current = Math.max(0, landingTimer.current - safeDelta);
    }
    ...
    if (
      animationGrounded &&
      isMoving &&
      landingAnimation.current === "Landing"
    ) {
      landingTimer.current = 0;
    }
    ...
    const isRunning =
      !isPushing && isMoving && keys.current.run && !isCrouching.current;
    ...
    const isWaitingForRunStop =
      ...
      !isCrouching.current &&
      runStopInputTimer.current > 0 &&
      runStopInputTimer.current < RUN_STOP_INPUT_GRACE &&
      currentAnimation.current === "Spint";
    ...
    const nextAnimation = resolvePlayerAnimation({
     ...
     */
    if (
      (nextAnimation === "Landing" || nextAnimation === "HardLanding") &&
      animationGrounded
    ) {
      velocityX = 0;
    }
     ...
     */
    wasGrounded.current = animationGrounded;
    ...
    effectState.locomotionActive =
      currentAnimation.current === "Jog" ||
      currentAnimation.current === "Spint" ||
      currentAnimation.current === "CrouchedSpinting" ||
      currentAnimation.current === "RunStop";
    ...
    <>
      ...
      >
        ...
        <CapsuleCollider
          ...
          args={[STANDING_HALF_HEIGHT, PLAYER_RADIUS]}
          position={[0, STANDING_COLLIDER_OFFSET_Y, 0]}
          friction={0}
        ...
        <CapsuleCollider
          ...
          args={[CROUCHING_HALF_HEIGHT, PLAYER_RADIUS]}
          position={[0, CROUCH_COLLIDER_OFFSET_Y, 0]}
          friction={0}
          frictionCombineRule={rapier.CoefficientCombineRule.Min}
        />
        ...
        <CuboidCollider
          ...
          position={[0, CEILING_SENSOR_OFFSET_Y, 0]}
          sensor
          onIntersectionEnter={({ other }) => {
          ...
          onIntersectionExit={({ other }) => {
            ...
            ceilingContacts.current = Math.max(0, ceilingContacts.current - 1);
          }}
        />
      ...
      <RunDustEffect stateRef={playerEffectState} />

      <SpeedLinesEffect stateRef={playerEffectState} />
    </>
  );
}
